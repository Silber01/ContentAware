...
class SeamEnergyWithBackPointer:
    ...
    """
    def __init__(self, energy, lastX):
        self.energy = energy
        self.lastX = lastX
...
def compute_vertical_seam_v2(energy_data):
    ...
    backPointerGrid = []
    backPointerGrid.append([])
    sumEnergyData = energy_data.copy()
    for y in range(1, len(energy_data)):
        backPointerRow = []
        for x in range(len(energy_data[0])):
            bestMove = getMinUp(sumEnergyData, x, y)
            moveEnergy = sumEnergyData[y][x] + sumEnergyData[y - 1][bestMove]
            thisSeamData = SeamEnergyWithBackPointer(moveEnergy, bestMove)
            backPointerRow.append(thisSeamData)
            sumEnergyData[y][x] += sumEnergyData[y - 1][bestMove]
        backPointerGrid.append(backPointerRow)
    lastRow = sumEnergyData[len(sumEnergyData) - 1]
    minIndex = lastRow.index(min(lastRow))
    totalEnergy = lastRow[minIndex]
    seam = [minIndex]
    bpIter = backPointerGrid[y][minIndex]
    for y in reversed(range(1, len(backPointerGrid))):
        bpIter = backPointerGrid[y][bpIter.lastX]
        seam.append(bpIter.lastX)
    seam.reverse()
    return seam, totalEnergy


def getMinUp(energy_data, x, y):
    left = energy_data[y - 1][max(x - 1, 0)]
    right = energy_data[y - 1][min(x + 1, len(energy_data[0]) - 1)]
    middle = energy_data[y - 1][x]
    minValue = min(left, right, middle)
    if minValue == middle:
        return x
    elif minValue == left:
        return x - 1
