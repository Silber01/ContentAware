...
def energy_at(pixels, x, y):
    ...
    leftCol = pixels[max(0, x - 1)][y]
    rightCol = pixels[min(x + 1, len(pixels) - 1)][y]
    upCol = pixels[x][max(0, y - 1)]
    downCol = pixels[x][min(y + 1, len(pixels[0]) - 1)]
    deltaX = diffSqrd(leftCol.r, rightCol.r) + diffSqrd(leftCol.g, rightCol.g) + diffSqrd(leftCol.b, rightCol.b)
    deltaY = diffSqrd(upCol.r, downCol.r) + diffSqrd(upCol.g, downCol.g) + diffSqrd(upCol.b, downCol.b)
    return deltaX + deltaY


def diffSqrd(num1, num2):
    return (num1 - num2) ** 2
...
def compute_energy(pixels):
    ...
    for i in range(len(pixels)):
        ...
        for j in range(len(pixels[i])):
            row.append(energy_at(pixels, i, j))
        energyValues.append(row)
    return energyValues
...
if __name__ == '__main__':
    ...
    print(f"length: {len(pixels)}, width: {len(pixels[0])}")
